...
変数 ```val``` が未定義または空文字列の場合は値 ```b``` を ```A ```に代入する。

```sh
A=${val-'b'}
```

変数 ```val``` が未定義または空文字列の場合は値 ```a``` を返却する。

```sh
${val:-a}
```

変数 ```val``` が未定義または空文字列の場合は変数 ```val``` に値 ```a``` を代入する。
...
変数 ```val``` が未定義または空文字列の場合はエラー終了する。

```sh
${val:?"error message"}
```

変数 ```val``` が未定義または空文字列以外の場合は値 ```a``` を返却する。
