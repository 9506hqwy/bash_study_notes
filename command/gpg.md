...
主鍵の用途の S, C はそれぞれ Sign (署名), Certify(承認) となる。
副鍵の用途の E は Encrypt(暗号化) となる。
...
対象ファイルに署名ファイルを埋め込む。パスワードの入力が求められる。
署名されたファイルの拡張子は *.asc* で新規に作成される。
`--armor` を指定しない場合は拡張子が *.gpg* でバイナリファイルが作成される。
...
pinentry-curses の場合はリモートホストにプロンプトが表示されてもパスワードを入力できなかった。
